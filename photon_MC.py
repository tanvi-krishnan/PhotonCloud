...
def generate_isotropic_direction(dim=3):
    if dim == 3:
        ...
        phi = 2 * torch.pi * torch.rand(1)
        direction = torch.tensor([torch.cos(phi), torch.sin(phi)])
    else:
...
def henyey_greenstein_scattering(g, current_direction, dim=3):
    ...
    if dim == 3:
        ...
        new_direction = torch.tensor([sin_theta * torch.cos(phi), sin_theta * torch.sin(phi)])
    else:
...
def photon_propagation_event_based_competing(num_photons, mu_s, mu_a, g, dim=3, max_time=20.0):
    data = []  # To store the results
    mu_t = mu_s + mu_a  # Total interaction rate

    for photon in range(num_photons):
        pos = torch.zeros(dim)  # Start at origin
        time_photon = 0.0
        event_data = []  # To store (position, time, delta_t) for each scattering event

        direction = generate_isotropic_direction(dim)
        while time_photon < max_time:
            ...
            if torch.rand(1).item() < (mu_s / mu_t):
                ...
                direction = henyey_greenstein_scattering(g, direction, dim=dim).flatten()
            else:
                break
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dim", type=int, choices=[2, 3], default=3, help="Dimensionality of the simulation (2D or 3D)")
